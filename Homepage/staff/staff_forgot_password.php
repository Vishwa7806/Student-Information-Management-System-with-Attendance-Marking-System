...
if (isset($_POST['submit'])) {
    ...
    if (empty($email) || empty($mobile_no) || empty($new_password) || empty($confirm_password)) {
        ...
        if (!preg_match($password_pattern, $new_password)) {
            ...
            $sql = "SELECT ID FROM staff_user WHERE Email = ? AND PhoneNo = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                if ($new_password === $confirm_password) {
                    ...
                    $staff_id = $result->fetch_assoc()['ID']; // Get staff ID from matched records
                    $sql = "UPDATE staff_user SET Password = ? WHERE ID = ?";
                    $stmt = $conn->prepare($sql);
